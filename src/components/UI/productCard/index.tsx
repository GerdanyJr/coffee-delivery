...
function ProductCard({ description, image, name, price, categories }: Coffee) {
  return (
    <div className="flex flex-col bg-base-card rounded-tl-md rounded-br-md rounded-tr-[36px] rounded-bl-[36px] w-min items-center font-roboto dark:bg-base-card_dark">
      ...
      <div className="flex w-[90%] flex-wrap justify-center mb-4 gap-1">
        {categories?.map((product) => (
          <span
            key={product.id}
            className="bg-yellow-light uppercase text-yellow-dark text-[10px] font-bold rounded-full py-1 px-2 dark:bg-purple-dark dark:text-white"
          >
            {product.name}
          </span>
        ))}
      ...
      <h2 className="mb-2 text-xl font-bold leading-snug font-baloo dark:text-white">{name}</h2>
  ...
  (
    ...
    return (
      ...
      >
        ...
        <h2 className="mb-2 text-xl font-bold leading-snug font-baloo">
          {name}
        </h2>
